...
import { RadarChart, ResultSidebar } from '@/components/results';
import {
  AlertCircle, CheckCircle, AlertTriangle, Lightbulb02,
  Target04, Stars01, Grid01, Lightbulb05, Clock, Globe01, Eye,
  Palette, TypeSquare, LayersThree01,
} from '@untitledui/icons';
import { Header } from '@/components/layout/Header';
import { TestBanner } from '@/components/analysis/visual/TestBanner';
...
import { MockupBentoBox } from '@/components/analysis/visual/MockupBentoBox';
...
export default function VisualAnalysisPage() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const res = await fetch(`/api/analysis/${id}/visual`);
        if (!res.ok) {
          if (res.status === 404) {
            setError('Vizuális elemzés nem található');
          } else {
        ...
        setVisualAnalysis(analysis.visual_analysis as VisualAnalysis);
        setLogoUrl(analysis.logo_url);
        setLogoName(analysis.logo_name);
        setCreatorName(analysis.creator_name ?? null);
        setCategory(analysis.category ?? null);
        setTier(analysis.tier ?? null);
        setCreatedAt(analysis.created_at ?? null);

        const score = (analysis.result as AnalysisResult)?.osszpontszam;
  ...
  if (error || !result || !visualAnalysis || !logoUrl) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-white px-4">
        <div className="max-w-md text-center">
          ...
          <h1 className="mb-2 text-2xl font-bold text-[#1f2937]">{error || 'Nem található'}</h1>
          <p className="mb-8 text-[#6b7280]">
            A vizuális elemzés nem elérhető vagy még nem lett legenerálva.
          </p>
          <a href={`/eredmeny/${id}`} className="inline-flex items-center gap-2 rounded-full bg-[#1f2937] px-6 py-3 font-medium text-white transition-all hover:bg-[#374151]">
  ...
  return (
    <div className="min-h-screen bg-[#f9fafb] overflow-x-clip">
      {/* Teszt banner */}
      <TestBanner analysisId={id} />
      ...
      <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        {/* FEJLÉC – logó + pontszám */}
        <div className="mb-8 flex flex-col sm:flex-row gap-6 sm:gap-8">
          <div className="shrink-0 flex items-center justify-center rounded-2xl border border-gray-200 bg-white w-full sm:w-[40%] aspect-square p-10">
            <img src={logoUrl} alt={logoName || 'Logó'} className="max-h-full max-w-full object-contain" />
          </div>
          <div className="flex-1 flex flex-col justify-center py-4">
            {logoName && (
              <h1 className="text-2xl font-semibold text-gray-900 mb-2">{logoName}</h1>
            )}
            <div className="flex items-baseline gap-1">
              <span className="font-bold text-[#37352f]" style={{ fontSize: '80px', lineHeight: '80px' }}>
                {displayScore}
              </span>
              <span className="font-normal text-[#9ca3af]" style={{ fontSize: '26px' }}>/100</span>
            </div>
            ...
            <p className="mt-3 text-[10px] text-gray-400 tracking-wider uppercase">powered by brandguide SCORE</p>
          </div>
        </div>

        {/* ÖSSZEFOGLALÓ */}
        <div className="mb-6 rounded-2xl border border-gray-100 bg-white p-6">
          <h2 className="mb-3 text-xl font-light text-gray-900">Összefoglaló</h2>
          <p className="text-[16px] leading-[26px] text-gray-500">{renderWithBold(result.osszegzes || '')}</p>
        </div>
        ...
        {/* RADAR CHART */}
        <div className="mb-6 rounded-2xl border border-gray-100 bg-white p-6">
          <h2 className="mb-4 text-xl font-light text-gray-900">Szempontok áttekintése</h2>
          <RadarChart result={result} />
        </div>

        {/* ⭐ GEOMETRIAI ELEMZÉS PANEL */}
        <GeometryPanel data={visualAnalysis.geometry} logoUrl={logoUrl} />

        {/* RÉSZLETES ÉRTÉKELÉS – tabbed */}
        <div className="mb-6">
          <h2 className="mb-4 text-xl font-light text-gray-900">Részletes értékelés</h2>
                ...
                return (
                  <>
                    <div className="mb-4 flex items-start justify-between">
                      <h3 className="text-xl font-light text-gray-900 flex items-center gap-1">
                        {meta.displayName}
                        {TOOLTIPS.criteria[activeTab as keyof typeof TOOLTIPS.criteria] && (
                          <InfoTooltip text={TOOLTIPS.criteria[activeTab as keyof typeof TOOLTIPS.criteria]} />
                        )}
                      </h3>
                    ...
                    <p className="mb-6 text-[16px] leading-[26px] text-gray-500">{value.indoklas}</p>
        ...
        {/* SZÍNPALETTA ELEMZÉS (AI szöveg) */}
        {result.szinek && (
          <div className="mb-6 rounded-2xl border border-gray-100 bg-white p-6">
            <div className="mb-5 flex items-center gap-3">
              <Palette className="size-5 text-purple-600" />
              <h3 className="text-xl font-light text-gray-900 flex items-center gap-1">Színpaletta elemzés<InfoTooltip text={TOOLTIPS.color.section} /></h3>
            </div>
        ...
        {result.tipografia && (
          <div className="mb-6 rounded-2xl border border-gray-100 bg-white p-6">
            <div className="mb-5 flex items-center gap-3">
              ...
              <h3 className="text-xl font-light text-gray-900 flex items-center gap-1">Tipográfia elemzés<InfoTooltip text={TOOLTIPS.typography.section} /></h3>
            </div>
            <div className="space-y-5">
              <div>
        ...
        {result.vizualisNyelv && (
          <div className="mb-6 rounded-2xl border border-gray-100 bg-white p-6">
            <div className="mb-5 flex items-center gap-3">
              ...
              <h3 className="text-xl font-light text-gray-900 flex items-center gap-1">Vizuális nyelv elemzés<InfoTooltip text={TOOLTIPS.visualLanguage.section} /></h3>
            </div>
        ...
        {/* ⭐ MOCKUP BENTO BOX */}
        <MockupBentoBox logoUrl={logoUrl} isTestMode={true} />
        ...
        <div className="rounded-2xl bg-gray-900 p-8 text-center sm:p-10">
          ...
          <a href="https://ai.brandguide.hu/" target="_blank" rel="noopener noreferrer">
            <button className="group inline-flex items-center gap-3 rounded-full bg-[#fff012] px-8 py-4 font-semibold text-gray-900 transition-all hover:brightness-95 hover:shadow-lg cursor-pointer">
