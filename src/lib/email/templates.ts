...
// Inline style for buttons (email clients may not support <style> tags)
const BTN_STYLE = 'display:inline-block;padding:12px 28px;background:#1a1a1a;color:#ffffff;text-decoration:none;border-radius:8px;font-weight:600;font-size:14px;';
...
// v0.91: Összevont fizetés sikeres + magic link
export function paymentSuccessWithMagicLinkEmail(params: {
  tierName: string;
  amount: string;
  date: string;
  magicLink: string;
...
export function approvalEmail(params: {
  ...
  const html = layout(`
    ...
    <p style="text-align:center;margin-top:24px;">
      <a href="${params.galleryUrl}" class="btn" style="${BTN_STYLE}">Megtekintés a galériában</a>
    </p>
...
export function weeklyWinnerEmail(params: {
  ...
  const html = layout(`
    ...
    <p style="text-align:center;margin-top:24px;">
      <a href="${params.galleryUrl}" class="btn" style="${BTN_STYLE}">Megtekintés</a>
    </p>
...
export function welcomeEmail(params: {
  ...
  const html = layout(`
    ...
    <p style="text-align:center;margin-top:24px;">
      <a href="${APP_URL}/elemzes/uj" class="btn" style="${BTN_STYLE}">Első elemzés indítása</a>
    </p>
...
export function reminderEmail(params: {
  ...
  const html = layout(`
    ...
    <p style="text-align:center;margin-top:24px;">
      <a href="${APP_URL}/elemzes/uj" class="btn" style="${BTN_STYLE}">Elemzés indítása</a>
    </p>
...
// Bejelentkezési magic link email
export function loginMagicLinkEmail(params: {
  magicLink: string;
}) {
  const subject = 'Bejelentkezés — LogoLab';
  const html = layout(`
    <p>Szia!</p>
    <p>Kattints az alábbi gombra a bejelentkezéshez:</p>
    <p style="text-align:center;margin:24px 0;">
      <a href="${params.magicLink}" class="btn" style="${BTN_STYLE}">Bejelentkezés</a>
    </p>
    <p style="font-size:12px;color:#999;">A link 15 percig érvényes. Ha nem te kérted ezt az emailt, nyugodtan hagyd figyelmen kívül.</p>
  `);
  return { subject, html };
}

// Regisztrációs megerősítő email
export function registrationConfirmEmail(params: {
  name: string;
  magicLink: string;
}) {
  const subject = 'Regisztráció megerősítése — LogoLab';
  const html = layout(`
    <p>Üdvözlünk, ${params.name}!</p>
    <p>Köszönjük a regisztrációt! Kattints az alábbi gombra a fiókod megerősítéséhez:</p>
    <p style="text-align:center;margin:24px 0;">
      <a href="${params.magicLink}" class="btn" style="${BTN_STYLE}">Fiók megerősítése</a>
    </p>
    <p style="font-size:12px;color:#999;">A link 15 percig érvényes. Ha nem te regisztráltál, nyugodtan hagyd figyelmen kívül.</p>
  `);
  return { subject, html };
}
